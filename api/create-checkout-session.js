...
function loadJSON(org, name){
  return JSON.parse(fs.readFileSync(path.join(process.cwd(),'data',org,name+'.json'),'utf-8'));
}
function computeSurcharge(base, settings){
  const S = settings?.surcharge||{}; if(!S.enabled) return 0;
  const P = Number(S.fee_percent||0), F=Number(S.fee_fixed_cents||0), CAP=Number(S.cap_percent||0);
  if(P<=0 && F<=0) return 0;
  const total = Math.ceil((base + F) / (1 - P));
  let sur = total - base;
  if(CAP>0){ const capByPct = Math.floor(base*CAP); if(sur>capByPct) sur = capByPct; }
  return sur;
}
...
export default async function handler(req, res){
  ...
  try{
    ...
    (order.attendees||[]).forEach(att => (att.selections||[]).forEach(sel => {
      if(sel?.handle && ticketMap.has(sel.handle)){
        const price = Number(ticketMap.get(sel.handle).price_cents||0);
        line_items.push({ price_data:{ currency:'usd', unit_amount:price, product_data:{ name:`Ticket (${sel.handle})`, description:`Meal:${sel.meal||''} | Diet:${sel.dietary||''}` } }, quantity: 1 });
        const s = computeSurcharge(price, settings); if(s>0) line_items.push({ price_data:{ currency:'usd', unit_amount:s, product_data:{ name:'Card processing fee' } }, quantity:1 });
      }
    }));
